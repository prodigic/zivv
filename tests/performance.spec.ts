...
test.describe("Performance Testing", () => {
  ...
  test("should load homepage within acceptable time limits", async ({
    page,
  }) => {
  ...
  test("should render events efficiently", async ({ page }) => {
    ...
    const renderingMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        eventCards: document.querySelectorAll(
          '.event-card, .event-item, [data-testid="event-card"]'
        ).length,
    ...
    if (renderingMetrics.eventCards > 0) {
      const elementsPerEvent =
        renderingMetrics.totalElements / renderingMetrics.eventCards;
  ...
  test("should handle search operations efficiently", async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      expect(incrementalTime).toBeLessThan(1200); // Adjusted threshold
  ...
  test("should maintain performance on mobile devices", async ({
    ...
    if (await mobileFilter.isVisible()) {
      ...
      const mobileInteractionTime =
        mobileInteractionEnd - mobileInteractionStart;
  ...
  test("should have minimal bundle size impact", async ({ page }) => {
    ...
    // CSS should be minimal (under 1MB)
    expect(cssSize).toBeLessThan(1024 * 1024);
