...
test.describe("Event Filtering and Display", () => {
  ...
  test("should display event cards when events are loaded", async ({
    page,
  }) => {
    ...
    if (eventCount > 0) {
      ...
      const titleElement = firstCard.locator(
        'h1, h2, h3, .title, [data-testid="event-title"]'
      );
  ...
  test("should handle empty state gracefully", async ({ page }) => {
    ...
    const emptyState = page.locator(
      '.empty-state, [data-testid="empty-state"], .no-events'
    );
    ...
    if (eventCount === 0) {
      ...
      const stateVisible = await loadingIndicator
        .or(emptyState)
        .or(errorState)
        .isVisible()
        .catch(() => false);
  ...
  test("should allow city-based filtering", async ({ page }) => {
    ...
    const cityFilters = page.locator(
      '[data-testid="city-filter"], .city-filter, .city-selector'
    );
  ...
  test("should allow date-based filtering", async ({ page }) => {
    ...
    const dateFilters = page.locator(
      '[data-testid="date-filter"], .date-filter, .date-selector'
    );
  ...
  test("should provide search functionality", async ({ page }) => {
    // Look for main search input in header (more specific selector)
    const searchInput = page
      .locator('header input[placeholder*="artists" i]')
      .first();
  ...
  test("should handle free show filtering", async ({ page }) => {
    ...
    const freeFilter = page.locator(
      'text=Free, [data-testid="free-filter"], .free-filter'
    );
  ...
  test("should allow clearing filters", async ({ page }) => {
    ...
    const cityFilter = page
      .locator('[data-testid="city-filter"], .city-filter')
      .first();
    ...
    if (await cityFilter.isVisible()) {
      ...
      const clearButton = page.locator(
        'text=Clear, [data-testid="clear-filters"], .clear-filters'
      );
  ...
  test("should toggle between list and calendar views", async ({ page }) => {
    ...
    const listView = page.locator(
      'text=List, [data-testid="list-view"], .list-view'
    );
    const calendarView = page.locator(
      'text=Calendar, [data-testid="calendar-view"], .calendar-view'
    );
    ...
    if ((await listView.isVisible()) && (await calendarView.isVisible())) {
      ...
      const calendar = page.locator(
        '.calendar, [data-testid="calendar"], .fc-view'
      ); // FullCalendar
      ...
      const list = page.locator(
        '.event-list, [data-testid="event-list"], .list'
      );
    }
  });
