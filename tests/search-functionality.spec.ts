...
test.describe("Search Functionality", () => {
  ...
  test("should have search input available", async ({ page }) => {
    // Look for main search input field in header
    const searchInput = page.locator('header input[placeholder*="artists" i]');
  ...
  test("should provide search suggestions or autocomplete", async ({
    page,
  }) => {
    const searchInput = page
      .locator('header input[placeholder*="artists" i]')
  ...
  test("should be accessible via keyboard", async ({ page }) => {
    ...
    for (let i = 0; i < 20 && !found; i++) {
      ...
      if (tagName === "input" && (type === "search" || type === "text")) {
        ...
        await focusedElement.selectText();
